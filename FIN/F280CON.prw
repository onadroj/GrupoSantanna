...
±±ºPrograma  ³F280CON   º Autor ³ GATASSE            º Data ³  30/05/03   º±±
...
//	@ 050, 036 SAY _CCUNID
	...
	ENDIF
	IF !_NOK
		aParcelas := Condicao(paramixb[1],_cCodPagto,,ddatabase)  //FUNCAO SIGA PARA MONTAR VETOR COM PARCELAMENTO
		_t:=0
		for _x:=1 to len(aParcelas)
			_t:=_t+aParcelas[_x,2]
		next
	   aParcelas[1,2]:=aParcelas[1,2]+paramixb[1]-_t  //AJUSTE NA PRIMEIRA PARCELA
	ENDIF
EndDo
Return   (aParcelas)
...
if _cret
	...
	_nDescCond:=ALLTRIM(SE4->E4_DESCRI)
    if SE4->E4_TIPO=="9"
      MSGSTOP("A condicao "+ALLTRIM(_cCodPagto)+"-"+_nDescCond+" nao gera parcelas!")
      _cret:=.F.
    else
   	  @ 004, 080 SAY _nDescCond
   	endif
